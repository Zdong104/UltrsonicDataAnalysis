...
close all
%% Load Sample Now
FileName = load(['Sample_Loose_1Mhz/Sample_Loose_1MHz_22.mat']);
A = [];
    A = FileName.A;
    for interval_Sample = 1:size(A)
        if A(interval_Sample)> 0.05
...
title("Raw data Collected from Probe for 1MHz Loose Sample")
saveas(figure(1),'Raw Data for 1MHz Loose Sample.jpg')
%%
...
desired_freq = 1*1000; % This is the desired frequency in kHz which is 1 MHz
figure(3)
...
ylabel('Amplitude')
 %% Curve fitting
        ...
        g_Sample = hann (windowLength_Sample);
        % this part been changed to hann function from the original gauss
        % function, there is not a lot of difference, but hann function
        % give more low frequency collection, here is the way you could
        % look on that wvtool(hann(L)) and wvtool(gauss(L)), where wvtool
        % is the visualize window. 
...
%% Load water data now
FileName = load('Water_Loose_1Mhz/Water_Loose_1MHz_22.mat');
A = [];
    A = FileName.A;
    for interval_water = 1:size(A)
        if A(interval_water)> 0.2
...
figure(7)
plot(A)
title('Raw data Collected from Probe for 1MHz Loose Water')
saveas(figure(7),'Raw Data for 1MHz Loose Water.jpg')% meaningless plot since it does not have any units
%%
...
ylabel('Amplitude')
 ...
 %% Curve fitting
        ...
        g_Water = hann (windowLength_Water);
...
TOFs = windowLength_Sample;
TOFr = windowLength_Water;
